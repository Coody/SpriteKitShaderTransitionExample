...
extension SKScene {
    ...
    private func createShader(shaderName: String, transitionDuration: NSTimeInterval) -> SKShader {
        ...
        let u_size = SKUniform(name: "u_size", floatVector3: GLKVector3Make(Float(UIScreen.mainScreen().scale * size.width), Float(UIScreen.mainScreen().scale * size.height), Float(0)))
        let u_fill_colour = SKUniform(name: "u_fill_colour", floatVector4: GLKVector4Make(131.0 / 255.0, 149.0 / 255.0, 255.0 / 255.0, 1.0))
        let u_border_colour = SKUniform(name: "u_border_colour", floatVector4: GLKVector4Make(104.0 / 255.0, 119.0 / 255.0, 204.0 / 255.0, 1.0))
    ...
    func presentScene(scene: SKScene?, shaderName: String, transitionDuration: NSTimeInterval) {
        ...
        dispatch_after(delayTime, dispatch_get_main_queue(), { () -> Void in
            let fadeOverlay = SKShapeNode(rect: CGRectMake(0, 0, self.size.width, self.size.height))
